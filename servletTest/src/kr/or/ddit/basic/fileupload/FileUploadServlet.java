...
/*
 	...
 	1. fileSizeThreshold : 이 곳에 지정한 크기가 넘어가면 디스크의
 		임시 디렉토리에 저장한다.(단위 : byte, 기본값: 0(즉, 무조건 임시디렉토리를 사용한다.))
 	2. maxFileSize : 1개의 파일의 최대 크기(단위: byte, 기본값: -1L(무제한))
 	3. maxRequestSize : 서버로 전송되는 request의  최대 크기
 			(단위: byte, 기본값: -1L(무제한))
 	4. location : Part.write()메서드를 통해서 파일을 저장할 디렉토리 지정
 			(기본값 : "")
*/
...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
			...
			2. request.getPart("Part이름"); ==> 지정한 이름을 가진
				개별 Part객체를 반환한다.
		*/
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {// 공백이 아니면 파일데이터이다
